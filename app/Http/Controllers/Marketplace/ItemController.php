...
class ItemController extends Controller
{
    ...
    public function store(StoreItemRequest $request)
    {
        ...
        $suspiciousWords = ['fake', 'scam', 'urgent', 'free money', 'hack', 'password'];

        foreach ($suspiciousWords as $word) {
            if (
                str_contains(strtolower($request->title), $word) ||
                str_contains(strtolower($request->description), $word)
            ) {
                $isFlagged = true;
                $flagReasons[] = 'Suspicious keyword: ' . $word;
            }
        }

        if ($request->price < 5) {
            $isFlagged = true;
            $flagReasons[] = 'Price too low';
        }

        if ($request->price > 100000) {
            $isFlagged = true;
            $flagReasons[] = 'Price unusually high';
        }

        $todayPostCount = Item::where('seller_id', auth()->id())
            ->whereDate('created_at', now()->toDateString())
            ->count();

        if ($todayPostCount >= 5) {
            $isFlagged = true;
            $flagReasons[] = 'Too many items posted today';
        }
        ...
        return redirect()
            ...
            ->with('success', $isFlagged ? 'Item posted but flagged for admin review.' : 'Item listed successfully!');
    }
